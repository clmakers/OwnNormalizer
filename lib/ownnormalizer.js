...
function ownNormalizer(options) {
...
//                        throw new Error("Duplicate rule file has been found");
    ...
    this.validate = function (valOptions, callback) {
        var responseObject = [];
        ...
        if (typeof valOptions.insideEncrypt !== 'undefined') {
            runTimeOptions.insideEncrypt = valOptions.insideEncrypt;
        } else {
            runTimeOptions.insideEncrypt = defaults.insideEncrypt;
        }
        ...
        if (errCount === 0)
        {
            utils.securityStrategy(options,runTimeOptions).encryptJwt(responseObject, function (err, response) {
                if (err)
                    callback(err, null);
                else
    ...
    this.encrypt = function (data, callBack) {
        utils.securityStrategy(options,runTimeOptions).encrypt(data, function (err, response) {
            if (err)
                callBack(err, null);
            else
    ...
    this.decrypt = function (data, callBack) {
        utils.securityStrategy(options,runTimeOptions).decrypt(data, function (err, response) {
            if (err)
                callBack(err, null);
            else
                callBack(null, response);
    ...
    this.encryptJwt = function (data, callBack) {
        utils.securityStrategy(options,runTimeOptions).encryptJwt(data, function (err, response) {
            if (err)
                callBack(err, null);
            else
                callBack(null, response);
    ...
    this.decryptJwt = function (data, callBack) {
        utils.securityStrategy(options,runTimeOptions).decryptJwt(data, function (err, response) {
            if (err)
                callBack(err, null);
            else
                callBack(null, response);
